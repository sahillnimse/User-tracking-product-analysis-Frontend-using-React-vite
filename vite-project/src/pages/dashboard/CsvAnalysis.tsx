...
//
// FIX (crash): was importing useCsvData from "@/context/CsvDataContext" —
//   the old standalone provider that is never mounted in the tree. Changed to
//   "@/context/AppContext" (the unified provider that IS mounted).
//
// FIX (wrong data): was deriving `data` from useAnalytics() which returns
//   live/demo data. CSV Analysis should ALWAYS show the uploaded CSV stats,
//   regardless of the selected data-source mode. Now builds display data
//   directly from ParsedStats returned by useCsvData().
...
// FIX: import from the unified AppContext, NOT the old CsvDataContext file
import { useCsvData }               from "@/context/AppContext";
import { cn }                       from "@/lib/utils";
import type { FeatureAdoptionItem, ParsedStats } from "@/types";
...
// ── Helper: build display-ready object from raw ParsedStats ──────────────────
// Keeps the component clean and avoids going through useAnalytics()
// which may be in demo/live mode instead of csv mode.
function statsToDisplay(s: ParsedStats) {
  return {
    ...
    zeroEngagement:     s.zeroEngagement,
    featureAdoption:    s.featureAdoption,
    featureDepthFunnel: s.featureDepthFunnel,
    signupsByMonth:     s.signupsByMonth,
    cohortRetention:    s.cohortRetention,
    engagementSegments: s.engagementSegments,
    planBreakdown:      s.planBreakdown,
  };
}

type CsvDisplay = ReturnType<typeof statsToDisplay>;
    ...
    <div className="space-y-2">
      <div
        ...
        className={cn(
          ...
          "rounded-lg border-2 border-dashed px-6 py-12 transition-all duration-200",
          dragging ? "border-primary bg-primary/5"
                   : "border-border/60 hover:border-primary/40 hover:bg-primary/5",
        )}
      >
        <div className={cn("flex h-12 w-12 items-center justify-center rounded-xl border transition-colors",
        ...
        <span className="flex items-center gap-2 font-medium">
          ...
          {missing > 0 && (
            <span className="rounded bg-amber-500/15 px-1.5 py-0.5 font-mono
              text-[10px] font-semibold text-amber-500">{missing} not detected</span>
          )}
        </span>
        ...
        <div className="grid grid-cols-2 gap-2 border-t border-border/60 px-4 py-3 sm:grid-cols-3">
          {Object.entries(detected).map(([metric, col]) => (
            <div key={metric} className="flex flex-col gap-0.5 rounded-md bg-muted/40 p-2">
              <span className="font-mono text-[10px] uppercase tracking-wide text-muted-foreground">{metric}</span>
              <span className={cn("truncate font-mono text-xs",
                col === "(not found)" ? "text-amber-500" : "text-primary")}>{col}</span>
            </div>
          ))}
        </div>
      )}
        ...
        return (
          <div key={step.label} className="flex items-center gap-3">
            ...
            <div className="relative flex-1">
              <div className="flex h-7 items-center overflow-hidden rounded-md bg-muted">
                <div
                  className={cn("h-full rounded-md transition-all duration-500",
                    i === steps.length - 1 ? "bg-primary/60" : "bg-primary")}
...
// ── Raw preview ───────────────────────────────────────────────────────────────
function DataPreview({ rows, columns }: { rows: Record<string, string>[]; columns: string[] }) {
  const [open, setOpen] = useState(false);
  return (
    <div className="overflow-hidden rounded-lg border border-border/60 bg-card">
      <button onClick={() => setOpen(!open)}
              ...
              <tr className="bg-muted/40">
                {columns.map(c => (
                  <th key={c} className="px-3 py-2 text-left text-[10px] uppercase
                    tracking-widest text-muted-foreground whitespace-nowrap">{c}</th>
                ))}
              </tr>
            </thead>
            <tbody>
...
function EmptyUploadState() {
  return (
    <div className="flex flex-col items-center gap-4 rounded-xl border-2 border-dashed
      border-border/60 bg-card p-16 text-center">
      <div className="flex h-16 w-16 items-center justify-center rounded-2xl
        border border-primary/20 bg-primary/10">
        <Gavel className="h-7 w-7 text-primary" />
      </div>
      <div>
        <p className="font-display text-lg font-semibold text-foreground">No data yet</p>
        <p className="mt-1 max-w-xs font-mono text-xs text-muted-foreground">
          Upload a LawgicHub user export CSV above to see your analytics —
          KPIs, charts, feature adoption, cohorts and more will appear here.
        </p>
      </div>
      <div className="grid grid-cols-3 gap-3 mt-2 w-full max-w-sm">
        {["KPI cards", "Funnel chart", "Cohort table", "Feature adoption", "Plan breakdown", "Signups trend"].map(label => (
          <div key={label} className="rounded-md border border-border/40 bg-muted/30 px-2 py-2">
            <div className="h-2 w-3/4 rounded bg-muted-foreground/20 mb-1.5" />
            <div className="h-1.5 w-1/2 rounded bg-muted-foreground/10" />
            <p className="mt-1.5 font-mono text-[9px] text-muted-foreground/60">{label}</p>
          </div>
        ))}
      </div>
    </div>
...
export default function CsvAnalysis() {
  // FIX: was "@/context/CsvDataContext" — that old file's provider is never
  //      mounted, so this hook would throw on every render. Now uses AppContext.
  const { uploadCSV, clearCSV, oversized, fileName, stats } = useCsvData();

  // FIX: build display data directly from raw ParsedStats, not from
  //      useAnalytics() which reflects the live/demo data-source mode.
  const data: CsvDisplay | null = stats ? statsToDisplay(stats) : null;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="font-display text-2xl font-semibold text-foreground">CSV Analysis</h1>
            <span className={cn(
              "rounded-full px-2 py-1 text-[10px] font-semibold uppercase tracking-wide",
              fileName ? "bg-primary/15 text-primary" : "bg-muted text-muted-foreground",
            )}>
              {fileName ? "CSV loaded" : "No file"}
            </span>
          </div>
          <p className="mt-1 font-mono text-sm text-muted-foreground">
            {fileName
              ? `${fileName} · ${data?.totalUsers.toLocaleString() ?? "—"} users`
      ...
      {/* Everything below only renders when a CSV is loaded */}
      {!fileName || !data ? (
        <EmptyUploadState />
      ) : (
        <>
          ...
          <div className="flex items-start gap-3 rounded-lg border border-amber-500/20
            ...
            <div className="space-y-0.5 text-xs text-amber-700 dark:text-amber-400">
              <p className="font-semibold">
                Key insight: {((data.zeroEngagement / data.totalUsers) * 100).toFixed(1)}% of users never used any feature
              </p>
              <p className="text-amber-600 dark:text-amber-500">
                Draft is the primary engagement driver ({data.featureAdoption[0]?.users ?? 0} users).
                Focus onboarding on the Draft feature to improve activation rate ({data.activation}%).
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-5">
            <div className="lg:col-span-3 rounded-lg border border-border/60 bg-card p-5">
              ...
              <p className="mb-4 mt-0.5 font-mono text-xs text-muted-foreground">Monthly signup trend</p>
              {data.signupsByMonth.length > 0 ? (
                ...
                <div className="flex h-[210px] items-center justify-center">
                  <p className="font-mono text-xs text-muted-foreground">No date column detected in CSV</p>
                </div>
              )}
            </div>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-5">
            <div className="lg:col-span-3 rounded-lg border border-border/60 bg-card p-5 space-y-4">
              ...
              <div className="space-y-4">
                {data.featureAdoption.map(item => (
                  <FeatureBar key={item.feature} item={item}
                    max={Math.max(...data.featureAdoption.map(f => f.users), 1)} />
                ))}
              </div>
            </div>
            ...
            <div className="lg:col-span-2 rounded-lg border border-border/60 bg-card p-5">
              ...
              <p className="mb-2 mt-0.5 font-mono text-xs text-muted-foreground">By total feature interactions</p>
                ...
                <thead>
                  <tr className="border-b border-border/60">
                    {["Cohort","Users","Returned","Return %","Activated","Activation %"].map(h => (
                      <th key={h} className="pb-2 pr-6 text-left font-semibold text-muted-foreground">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
          ...
          {stats && (
            <>
              <ColumnMapping detected={stats.detectedCols} />
              {stats.rows.length > 0 && <DataPreview rows={stats.rows} columns={stats.columns} />}
            </>
          )}
        </>
      )}
