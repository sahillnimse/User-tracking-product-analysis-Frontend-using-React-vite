    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="font-mono text-xs text-muted-foreground">
          Behavioral cohorts and zero-engagement diagnostics
        </p>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <PanelCard title="Engagement segmentation" subtitle="Distribution of all signed-up users">
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie data={segmentBreakdown} dataKey="value" nameKey="name" outerRadius={110} label>
          ...
          <div className="mt-2 grid grid-cols-2 gap-2">
            {segmentBreakdown.map((s) => (
              <div key={s.name} className="flex items-center justify-between rounded border border-border/60 bg-muted/20 px-2 py-1.5 font-mono text-xs">
        ...
        <div className="space-y-4">
          <div className="rounded-lg border border-destructive/40 bg-destructive/5 p-4">
            ...
            <div className="mt-2 font-display text-3xl font-bold text-destructive">
              {zeroEngagement.total.toLocaleString()}
            </div>
            <div className="mt-1 font-mono text-xs text-muted-foreground">
              users signed up but never performed a single tracked action ({zeroEngagement.pctOfSignups}% of signups)
            </div>
          </div>

          <PanelCard title="Zero-engagement by acquisition source" subtitle="Where these users come from">
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={zeroEngagement.bySource}>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                <XAxis dataKey="source" stroke="hsl(var(--muted-foreground))" fontSize={10} />
                <YAxis stroke="hsl(var(--muted-foreground))" fontSize={10} />
                <Tooltip contentStyle={tooltipStyle} />
                <Bar dataKey="users" fill="hsl(0 75% 55%)" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </PanelCard>
        </div>
